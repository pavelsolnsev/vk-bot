...
export function createMessageNewHandler({ vk, store }) {
  return async (context) => {
    ...
    if (text === '+' || text === '-') {
      ...
      await tryPlusMinus({ vk, store, context, event: lastEvent, text, senderId })
      return
    }
    ...
    const startResult = await tryStartEvent({ vk, store, context, text, peerId, senderId })
    if (startResult) {
      await deleteIncomingCommandMessage(context)
    ...
    if (await tryAddTestPlayers({ vk, store, context, event: lastEvent, text })) {
      await deleteIncomingCommandMessage(context)
      return
    }
    ...
    if (await tryUnpayByNumber({ vk, store, context, event: lastEvent, text })) {
      await deleteIncomingCommandMessage(context)
      return
    }
    if (await tryRemoveByNumber({ vk, store, context, event: lastEvent, text })) {
      await deleteIncomingCommandMessage(context)
      return
    }
